        ...
        if (order.suborder === "entities") {
            for (i in order.data) {
                ...
                if (!entities.hasOwnProperty(entityState.id)) {
                    ...
                    entities[entityState.id] = new Entity(pos, `games/${gameName}/${sprite}`);
                }
                else
...
function onOrder(event) {
    ...
    if (state == "gameState") {
        console.log(`Got game state, tick duration is ${order.tick_duration}, game name is ${order.game_name}`);
        gameState = order;
        gameName = gameState.game_name;
        state = "playerState";
    }
    ...
    else if (state == "entitiesState") {
        entitiesState = order;
        state = "loadingGame";
        loadScript(`games/${gameName}/main.js`, function () {
            gameInit();
        ...
        for (o in waitingOrders)
            treatOrder(o);
        treatOrder(entitiesState);
        treatOrder(order);
        state = "playing";
    }
